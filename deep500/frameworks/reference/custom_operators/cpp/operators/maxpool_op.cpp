...
//currently supports only 1-d, 2-d, and 3-d maxpooling (i.e. 3-d, 4-d, 5-d tensors)

template<typename T>
class MaxPoolLayer : public deep500::CustomOperator{
...
public: 
    ...
    MaxPoolLayer(
        ...
        /*
        currently supports only 1d, 2d, and 3d maxpooling

        maxpool_1d: 3d-tensor
        maxpool_2d: 4d-tensor
        maxpool_3d: 5d-tensor
        */
                                        ...
                                        if(l_input_temp >= 0 && l_input_temp < m_input_shape[3]) {
                                            for(size_t w = 0; w < m_kernel_shape[2]; w++) {
                                                ...
                                                if(m_input_temp >=0 && m_input_shape[4]) {
                                                    ...
                                                    if(output_tensor[temp_index] < temp){
                                                        output_tensor[temp_index] = temp;
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        if(l_input_temp >= 0 && l_input_temp < m_input_shape[3]) {
                                            for(size_t w = 0; w < m_kernel_shape[2]; w++) {
                                                ...
                                                if(m_input_temp >=0 && m_input_temp < m_input_shape[4]) {
                                                    temp_index1 = utilityTensor::compute_index(
                                                        m_input_shape, {i, j,
                                                        (size_t)k_input_temp,
                                                        (size_t)l_input_temp,
                                                        (size_t)m_input_temp}
                                                    );
                                                    temp = fwd_input_tensor[temp_index1];
                                                    temp_index2 = utilityTensor::compute_index(
                                                        m_output_shape, {i, j, k, l, m}
                                                    );
                                                    if(fwd_output_tensor[temp_index2] == temp) {
                                                        input_tensor_grad[temp_index1] += nextop_grad[temp_index2];
                                                    }
                                                }
                                            }
                                        }
...
D500_EXPORTED void *create_new_op(deep500::tensor_t *input_descriptors, 
    ...
    return new MaxPoolLayer<DTYPE>(input_tensor_shape, auto_pad, kernel_shape, pads, strides);
}

D500_REGISTER_OP(MaxPoolLayer<DTYPE>);
...
does currently only support exactly 2 input tensors
*/

template<typename T>
class SumLayer : public deep500::CustomOperator{
...
public: 
    //constructor
    SumLayer(std::deque<uint32_t> input_shape) :
...
D500_EXPORTED void *create_new_op(deep500::tensor_t *input_descriptors, 
    ...
    return new SumLayer<DTYPE>(input_tensor_shape);
}

D500_REGISTER_OP(SumLayer<DTYPE>);